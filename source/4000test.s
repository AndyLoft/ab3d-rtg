...
*************************************************************
**Shoehorn stsrtup code in here ;AL
*************************************************************
	movem.l	d0-d7/a0-a6,-(a7)	; Put all regs on stack
	move.l	$4.w,a6				; exec
	jsr	-132(a6)				; forbid in exec, could also be -$84
	moveq	#0,d0
	lea	GFXname(pc),a1			; graphics.library in to a1
	jsr	OpenLib(a6)				; OpenLibrary(), could also be -$228 or -552
	beq.w	Error
	move.l	d0,a6
	move.l	a6,GFXbase
	move.l	34(a6),Actiview		; Get active viewport
	sub.l	a1,a1				; Find current task
	jsr	LoadView(a6)				; LoadView() -222
	jsr	WaitTOF(a6)				; WaitTOF() -270
	jsr	WaitTOF(a6)				; WaitTOF() 
	jsr	WaitTOF(a6)				; there must be atleast 4 WaitTOF's to
	jsr WaitTOF(a6)				; sync display correctly on 060/CV64!
*************************************************************

 include "ab3:source2/add/protsetupdecode"
; include "ab3:source2/add/ProtectionProtection"
; include "macros.i"
 include "ab3:source2/add/macros.s"
 include "ab3:source2/add/defs.i"
...
Error:
	move.l	$4.w,a6
	jsr	-$8a(a6)		; Permit()
	movem.l	(a7)+,d0-d7/a0-a6	; Get stored regs
	moveq	#20,d0			; "20" = Fatal error
 rts
*************************************************************
**Bits for startup code ;AL
*************************************************************
...
****************************
;	INCLUDE	"ab3:source2/add/chunkyconvert.s";AL
;	INCLUDE	"ab3:source2/add/kalms_CPU5BLIT0.s";AL
****************************
 jsr INITPLAYER
; bsr initobjpos
****************************
...
; move.l #bigfield,$dff080    ; Point the copper at our copperlist.;AL
...
nodrawp2:
;	jsr	ChunkyConvert	;Chunky conversion WIP AL
...
PAUSEOPTS:
 include "ab3:source2/add/pauseopts.s"
...
SplitHeight: dc.l 0
 ...
 include "ab3:source2/add/OrderZones"
...
***********************************
 include "ab3:source2/add/CD32JOY"
...
******************************
 include "ab3:source2/add/ObjectMove"
 include "ab3:source2/add/Anims"
******************************
startpass:
; include "ab3:source2/add/password_reloc.s"
endpass:
...
********** WALL STUFF *******************************
 ...
 include "ab3:source2/add/wallroutine3.chipmem"
...
gourfloor: dc.w 0
 ...
 include "ab3:source2/add/bumpmap.s"
...
********************************
*
 include "ab3:source2/add/ObjDraw3.chipram"
*
********************************
...
PLR2_Ducked: dc.b 0
 ...
 include "ab3:source2/add/PLR1CONTROL.s"
 include "ab3:source2/add/PLR2CONTROL.s"
 include "ab3:source2/add/FALL.s"
...
storeval: dc.w 0
 ...
 include "ab3:source2/add/wallchunk.s"
 include "ab3:source2/add/loadfromdisk.s"
 include "ab3:source2/add/screensetup.s"
 include "ab3:source2/add/CONTROLLOOP.s"
...
*****************************************************************
 *
 include "ab3:source2/add/LevelData2"
 *
*****************************************************************
...
; include "ab3:source2/add/loadmod.a";AL removed due to poor playback
; include "ab3:source2/add/proplayer.a";AL removed due to poor playback
...
nullline:
 ...
 include "ab3:source2/add/titlecop.s"
...
PROTCALC:
;	include "ab3:source2/add/protcalc.s"
